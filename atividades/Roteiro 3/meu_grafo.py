...
class MeuGrafo(GrafoListaAdjacencia):
    ...
    def criarlistavertices(self, dfs=False, bfs=False):
        """
    ...
    def bfs(self, V='',**kwargs):
        ...
        self.criarlistavertices(bfs=True)
        for i in range(len(self.lista_vertices)):
        ...
        for i in range(len(self.lista_vertices)):
            if(self.lista_vertices[i][2] in asv) and (self.verificar(**{"arvore":self._arvorebfs}) == False) and self.lista_vertices[i][2] not in self.asv:
    ...
    def verificar(self,**kwargs):
        _aux = []
        kwargs["arvore"].criarlistavertices()
